...
2. Tray → **LINK** (or `Ctrl+``) — judge console for all 25 tools; headline tools come with ready-made example inputs prefilled
...
6. Pick `show_in_document` — prefilled `{"path":"/Research/ORPHEUS/anomaly_notes.txt","query":"02:13 is not a time"}` → **EXECUTE** → the document opens, scrolls to line 145, briefly flashes, then **pins a persistent highlight** on the passage. Click anywhere in the document, scroll, or press the `◆ DISMISS` button in the toolbar to clear it.
...
6. Say: *“Show me where Daniel says 02:13 is not a time.”* → agent must call `show_in_document({"path":"/Research/ORPHEUS/anomaly_notes.txt","query":"02:13 is not a time"})` → document opens, scrolls to line 145, pins a persistent highlight, chat does **not** contain the passage (you read it on screen)
...
- [ ] `LINK` lists 25 tools with ◇ readOnly / ◆ nav / ⚑ untrusted, `title`/`description`/`inputSchema` visible, budgets enforced
- [ ] `get_system_logs filter 02:13` returns `log_035` with `gait mismatch` note
- [ ] `show_in_document` visibly opens + scrolls + pins a persistent highlight, does NOT dump paragraph in chat, and the pin clears on first click/scroll/type/close
