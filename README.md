...
// Real tools: 25 narrow tools (get_investigation_context, search_files, open_file, show_in_document, get_timeline, …)
...
- The McDuff case is Instance 1. The pattern (25 narrow tools + visible actuation) ports to any co-op investigation — newsroom, classroom archive, SOC — fork `src/game/data/*` and your community has its own accompanied desk
...
Agent:   calls search_messages, show_in_document...
...
- Opens relevant evidence on screen (`show_in_document`, `open_email`, etc.) and updates the shared Evidence board
...
## How the agent operates the computer — 25 narrow, auditable tools
...
| Navigation | `open_application`, `focus_application`, `open_file`, `show_in_document`, `open_directory`, `open_email`, `open_browser_entry`, `terminal_command` |
...
**Fastest verification (30 sec, no agent):** tray **LINK** (or `Ctrl+``) → pick `get_system_logs` → `{"filter":"02:13"}` → **EXECUTE** → watch the 02:13 login block appear. Then `show_in_document` with `{"path":"/Research/ORPHEUS/anomaly_notes.txt","query":"02:13 is not a time"}` → **EXECUTE** → the document opens, scrolls to line 145, and pins a persistent highlight on the passage (line 145 is the "02:13 is not a time" line). Or press **⚡ QUICK VERIFY** for the one-click 9 evals + 3 visible-actuation tool calls (`✅ WEBMCP VERIFIED`).
...
2. On the desktop, click the tray **LINK** button (also Ctrl+\`) to open the **Agent Link** panel — a judge/dev console listing all 26 registered tools with live execute. Filter by ◇ readOnly / ◆ nav / ⚑ untrusted. Pick any tool — headline tools arrive with example inputs prefilled — press EXECUTE, and watch the computer respond.
   ...
   - `show_in_document` — open + scroll + pin a persistent highlight (replaces the older `open_file` + `scroll_document_to_line` + `find_text_in_document` chain)
